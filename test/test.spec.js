...
test("encanto test", async () => {
    const response = await request(app)
        ...
        .expect(200);
    expect(response.text.includes(movies[0].attributes.title)).toBeTruthy();
...
test("isle of dogs test", async () => {
    const response = await request(app)
        ...
        .expect("Content-type", "text/html; charset=utf-8")
        .expect(200);
    expect(response.text.includes(movies[1].attributes.title)).toBeTruthy();
...
test("shawshank test", async () => {
    const response = await request(app)
        ...
        .expect("Content-type", "text/html; charset=utf-8")
        .expect(200);
    expect(response.text.includes(movies[3].attributes.title)).toBeTruthy();
...
test("totoro test", async () => {
    const response = await request(app)
        ...
        .expect("Content-type", "text/html; charset=utf-8")
        .expect(200)        
    expect(response.text.includes(movies[2].attributes.title)).toBeTruthy();
